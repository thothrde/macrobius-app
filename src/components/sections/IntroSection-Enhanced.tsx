...
import { oracleConnectionManager, ConnectionStatus } from '@/lib/enhanced-oracle-connection-manager';
import { 
  BookOpen, 
  Star, 
  Globe, 
...
const ConnectionStatus: React.FC<{
  ...
  const getStatusColor = () => {
    ...
    return oracleConnectionManager.getStatusColor(connectionState);
  };
  ...
  const getStatusMessage = () => {
    ...
    return oracleConnectionManager.getStatusMessage(connectionState);
  };
...
export const IntroSection: React.FC<IntroSectionProps> = ({ language, onNavigateToSection }) => {
  ...
  const getFeatures = () => {
    const aiAvailable = connectionState ? oracleConnectionManager.areAISystemsAvailable(connectionState) : true;
  ...
  const testConnection = async () => {
    ...
    try {
      const result = await oracleConnectionManager.testConnection(attempts);
